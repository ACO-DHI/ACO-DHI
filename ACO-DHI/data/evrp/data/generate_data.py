...
def generate_evrp_data(dataset_size, num_loc, num_station, capacities=None):

    stations = np.zeros((dataset_size, num_station, 2), dtype=np.float32)
    for i in range(num_station):
        if i % 4 == 0:
                stations[:,i] = np.concatenate((
                    np.random.uniform(low=0.0, high=0.5, size=(dataset_size,  1)).astype(np.float32),
                    np.random.uniform(low=0.0, high=0.5, size=(dataset_size,  1)).astype(np.float32),
                    ),axis=-1,)
        elif i % 4 == 1:
                stations[:,i] = np.concatenate((
                    np.random.uniform(low=0.0, high=0.5, size=(dataset_size,  1)).astype(np.float32),
                    np.random.uniform(low=0.5, high=1.0, size=(dataset_size,  1)).astype(np.float32),
    ...
    return {
        ...
        "locs": np.random.uniform(size=(dataset_size, num_loc, 2)).astype(np.float32),  # Node locations
        "demand": np.random.uniform(size=(dataset_size, num_loc)).astype(np.float32),  # Demand, uniform integer 1 ... 9
        "stations": stations,
    }  # Capacity, same for whole dataset
...
def generate_dataset(
    ...
    station_sizes: Union[int, List[int]] = [8,9,9],
    vehicles:Union[int, List[int]]=[4,5,8],
...
def generate_default_datasets(data_dir, generate_eda=False):
    ...
    generate_dataset(data_dir=data_dir, name="test", problem="evrp", seed=1234)
