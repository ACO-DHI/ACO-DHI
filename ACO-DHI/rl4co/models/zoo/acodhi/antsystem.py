...
class AntSystem:
    ...
    def calculate_distance(self, td_initial):
        ...
        return torch.log(reciprocal_distance_matrix)
    ...
    def run(
        ...
        """
        distance_matrix = self.calculate_distance(td_initial)
     
        for _ in range(n_iterations):
          
            td = td_initial.clone()
         
            actions, reward,best_actions=self._one_step(td, env,distance_matrix)
        ...
        print(self.final_reward)

        return td, action_matrix, self.final_reward

    def _one_step(self, td: TensorDict, env: RL4COEnvBase, dis: Any,):
        ...
        """
        # sampling
        td, env, actions, reward = self._sampling(td, env,dis)
        ...
        best_index,best_actions=self._update_results(actions, reward)
      


        # update pheromone matrix
        self._update_pheromone(actions, reward,td)

        return actions, reward,best_actions
    ...
    def _sampling(
        ...
        # heatmaps_logits = self.alpha * torch.log(self.pheromone) + self.beta * self.log_heuristic  + self.gama * torch.log(dis)
        ...
        while not td["done"].all():
            ...
            logits, mask = NonAutoregressiveDecoder.heatmap_to_logits(td, heatmaps_logits, num_starts)
    ...
    def _update_results(self, actions, reward):
        ...
        return best_index,best_actions

    def _update_pheromone(self, actions, reward,td):
    # calculate Δpheromone
        ...
        best_reward ,best_idx= mapped_reward.max(dim=1)
        ...
        if self.elitist:
                    delta_pheromone[
                        ...
                        from_node[:, best_idx].flatten(),
                        to_node[:, best_idx].flatten(),
                    ] += best_reward[batch_action_indices]  # Extra pheromone for best paths
        ...
        delta_pheromone[:, :, :td["num_station"][0] + 1] = delta_pheromone[:, :, :td["num_station"][0] + 1]*self.decay1
