...
class EVRPInitEmbedding(nn.Module):
    ...
    def __init__(self, embedding_dim, linear_bias=True, node_dim: int = 3):
        ...
        self.init_embed_station = nn.Linear(
            2, embedding_dim, linear_bias
        )  # depot embedding
    def forward(self, td):
        # [batch, 1, 2]-> [batch, 1, embedding_dim]
        depot,stations, cities = td["locs"][:, :1, :],td["locs"][:, 1:-td["demand"].size(-1), :], td["locs"][:, -td["demand"].size(-1):, :]
        ...
        # out1 = station_embeddings.unsqueeze(1).repeat(1, out.shape[1], 1)
