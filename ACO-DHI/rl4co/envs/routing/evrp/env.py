...
class EVRPEnv(RL4COEnvBase):
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        selected_demand = gather_by_index(
            demand,
            torch.clamp(current_node - self.generator.num_station, 0, n_loc),
            squeeze=False,
        )
        ...
        used_length = (
            ...
            + td["used_length"]
        ) * (current_node > self.generator.num_station).float()
        visited = torch.nn.functional.pad(td["visited"], (1, 0), mode="constant", value=0)
        visit = visited.scatter(
            -1, torch.clamp(current_node - self.generator.num_station, 0, n_loc), 1
        )[:, 1:]
        # SECTION: get done
        ...
        td_reset = TensorDict(
            {
                ...
                "max_length": torch.full(
                    (*batch_size,1), self.generator.max_length, device=device
                ),
                "num_station": torch.full(
                    (*batch_size,), self.generator.num_station, device=device
                ),
                "visited": torch.zeros(
    ...
    def get_action_mask(td: TensorDict) -> torch.Tensor:
        ...
        length_loc = (
            (
                ...
                - td["locs"][:, 1 + td["num_station"][0].item() :]
            ).norm(p=2, dim=-1)[:, :, None]
            + (
                td["locs"][:, 1 + td["num_station"][0].item() :, None]
                - td["locs"][:, None, : 1 + td["num_station"][0].item()]
            ).norm(p=2, dim=-1)
            + td["used_length"][:, :, None]
        )
        exceeds_length = (length_loc + 1e-4 > td["max_length"][0].item()).all(-1)
        ...
        exceeds_station = length_other[:, 1:] > td["max_length"][0].item()
        mask_station = exceeds_station | (
            td["current_node"] <= td["num_station"][0].item()
        ).expand_as(exceeds_station) 
        visited_all = td["visited"].bool().all(-1, keepdim=True).squeeze(-1)


        enough_battery_to_return = (dist + td["used_length"])[:,0] <= td["max_length"][0].item()
        condition1 = visited_all & enough_battery_to_return
        mask_station[condition1, :] = 1
        # Cannot visit the depot if just visited and still unserved nodes
        exceeds_depot = length_other[:, :1] > td["max_length"][0].item()
        mask_depot = (
    ...
    def _get_reward(self, td: TensorDict, actions: TensorDict) -> TensorDict:
        ...
        if "factor" in td.sorted_keys:
            return -get_tour_length(locs_ordered) * td["factor"]
        else:
    ...
    def check_solution_validity(td: TensorDict, actions: torch.Tensor):
        ...
        for i in range(actions.size(1)):
            ...
            assert (used_cap <= td["vehicle_capacity"]).all(), "Used more than capacity"
        ...
        for j in range(actions.size(1)):
            used_len += distance[:, j]
            assert (used_len <= td["max_length"]).all(), "Used more than energy"
