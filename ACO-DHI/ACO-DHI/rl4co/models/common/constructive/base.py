...
class ConstructivePolicy(nn.Module):
    ...
    def __init__(
        ...
        val_decode_type: str = "multistart_sampling",
        ...
        **unused_kw,
        ...
        self.project_graph = nn.Linear(128, 128, bias=False)
        self.project_node = nn.Linear(128, 128, bias=False)
    ...
    def forward(
        ...
        **decoding_kwargs,
        ...
        final_h = refiner(td,hidden)
        heatmap_logits=self.decoder(td,final_h)#改动
        # heatmap_logits = torch.matmul(heatmap_logits,torch.log(distance_matrix))
        heatmap_logits = heatmap_logits+distance_matrix

        # hidden, init_embeds = self.encoder(td)
        # # final_h = refiner(td,hidden)
        # heatmap_logits=self.decoder(td,hidden)#改动
        # # heatmap_logits = torch.matmul(heatmap_logits,torch.log(distance_matrix))
        ...
        td, env, heatmap_logits = self.decoder.pre_decoder_hook(td, env, heatmap_logits, num_starts)
        # heatmap_logits = (heatmap_logits + self.log_heuristic.transpose(1, 2)) * 0.5
        ...
        while not td["done"].all():
            # final_h = refiner(td,hidden)
            # heatmap_logits=self.decoder(final_h)
            
            logits, mask = NonAutoregressiveDecoder.heatmap_to_logits(
                td, heatmap_logits, num_starts
            )
            ...
            td = env.step(td)["next"]
            # hidden, init_embeds = self.encoder(td,emb)
        # final_h = refiner(td,hidden)
            # heatmap_logits=self.decoder(td,hidden)#改动
            # final_h = refiner(td,hidden)
            # heatmap_logits=self.decoder(td,final_h)
